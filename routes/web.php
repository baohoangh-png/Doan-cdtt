...
Route::prefix('admin')->middleware('auth')->group(function () {
    ...
    // Quản lý danh mục
    Route::prefix('categories')->name('cate.')->middleware(RoleMiddleware::class . ':0')->group(function () {
    ...
    // Đã có prefix 'admin' ở ngoài rồi, nên chỉ cần 'brands'
    Route::prefix('brands')->name('brand.')->middleware(RoleMiddleware::class . ':0')->group(function () {
    ...
    // Các route dành riêng cho admin cấp cao (role = 0)
    Route::middleware(RoleMiddleware::class . ':0')->group(function () {
        // Nếu bạn muốn một dashboard riêng cho role 0:
        // Route::get('/dashboard-role0', [DashboardController::class, 'indexForRole0'])->name('ad.dashboard.role0');
        // Nếu bạn muốn định nghĩa lại categories cho role 0 (cẩn thận với xung đột route):
        // Route::resource('categories', CategoryController::class)->names('ad.categories.role0');
        // Tốt nhất là sử dụng một URL khác hoặc một Controller khác nếu có sự khác biệt.
    });
}); // <--- Đóng group prefix('admin') và middleware('auth') ở đây
